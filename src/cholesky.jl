...
function cholesky_classic!(M::StridedMatrix{T}, a::Vector{T}, b::Vector{T}, block_size::Int=2^7, block_size2::Int=2^9, block_size3::Int=2^10, block_size4::Int=2^11) where {T<:AbstractFloat}
    n = size(M, 1)
    ...
    @inbounds for iv0 in 1:block_size4:n
        iv1 = min(n, iv0 + block_size4 - 1)

        for iii0 in iv0:block_size3:iv1
            iii1 = min(iv1, iii0 + block_size3 - 1)

            for ii0 in iii0:block_size2:iii1
                ii1 = min(iii1, ii0 + block_size2 - 1)
                ...
                if ii1 < iii1
                    ldiv!(UpperTriangular(view(M, ii0:ii1, ii0:ii1))',
                        view(M, ii0:ii1, (ii1+1):iii1))
                    mul!(view(M, (ii1+1):iii1, (ii1+1):iii1),
                        transpose(view(M, ii0:ii1, (ii1+1):iii1)),
                        view(M, ii0:ii1, (ii1+1):iii1),
                        -one(T), one(T))
                end
            end
            ...
            if iii1 < iv1
                ldiv!(UpperTriangular(view(M, iii0:iii1, iii0:iii1))',
                    view(M, iii0:iii1, (iii1+1):iv1))
                mul!(view(M, (iii1+1):iv1, (iii1+1):iv1),
                    transpose(view(M, iii0:iii1, (iii1+1):iv1)),
                    view(M, iii0:iii1, (iii1+1):iv1),
                    -one(T), one(T))
            end
        end
        ...
        if iv1 < n
            ldiv!(UpperTriangular(view(M, iv0:iv1, iv0:iv1))',
                view(M, iv0:iv1, (iv1+1):n))
            mul!(view(M, (iv1+1):n, (iv1+1):n),
                transpose(view(M, iv0:iv1, (iv1+1):n)),
                view(M, iv0:iv1, (iv1+1):n),
                -one(T), one(T))
        end
    end
...
function cholesky_rowmax!(M::StridedMatrix{T}, a::Vector{T}, b::Vector{T}, block_size::Int=2^7, block_size2::Int=2^9, block_size3::Int=2^10, block_size4::Int=2^11) where {T<:AbstractFloat}
    n = size(M, 1)
    perm = collect(1:n)
    rank = n
    d_vec = zeros(T, n)
...
function cholesky_genz!(M::StridedMatrix{T}, a::Vector{T}, b::Vector{T}, block_size::Int=2^7, block_size2::Int=2^9, block_size3::Int=2^10, block_size4::Int=2^11) where {T<:AbstractFloat}
    c2 = T(1.0 / sqrt(2.0))
