...
function run_pnorm_sparse_just(Σ, a, b; max_pts=2^10, seed=0)
...
function run_pnorm_sparse32_just(Σ, a, b; max_pts=2^10, seed=0)
    nnz_mul = count(abs.(Σ .> eps(Float64))) / prod(size(Σ)) < 0.1 ? 4 : 1
...
function compare_just_sparse_methods(;
    n_dim=1000,
    n_reps=20,
    seed=42,
    qmc_pts=9600,
    ...
    for rep in 1:n_reps
        val, err, t = run_pnorm_sparse_just(
            M,
            a,
            b;
    ...
    for rep in 1:n_reps
        val, err, t = run_pnorm_sparse32_just(
            M,
            a,
            b;
            max_pts=ceil(Int, qmc_pts / 12),
...
function summarize_just_sparse_results(df::DataFrame)
    if isempty(df)
...
function run_all_just_sparse_simulations(ns, n_ptss, n_reps; mat_fun)
    val_df = DataFrame()
    t_df = DataFrame()

    @showprogress for n in ns
        for n_pts in n_ptss
            results = compare_just_sparse_methods(
                n_dim=n,
                qmc_pts=n_pts,
                n_reps=n_reps,
                mat_fun=mat_fun,
            )
            val_df_i, t_df_i = summarize_just_sparse_results(results)
...
just_sparse_vals, just_sparse_times = run_all_just_sparse_simulations(
    ns,
    n_ptss,
    n_reps;
    mat_fun=mattern_cov1,
)
just_sparse_vals[!, :matrix] .= "mattern_cov1"
just_sparse_times[!, :matrix] .= "mattern_cov1"

just_dense_vals, just_dense_times = run_all_just_sparse_simulations(
    ns,
    n_ptss,
    n_reps;
    mat_fun=mattern_cov2,
)
just_dense_vals[!, :matrix] .= "mattern_cov2"
just_dense_times[!, :matrix] .= "mattern_cov2"

just_fixed_dense_vals, just_fixed_dense_times = run_all_just_sparse_simulations(
    ns,
    n_ptss,
    n_reps;
    mat_fun=fixed_dense,
)
just_fixed_dense_vals[!, :matrix] .= "fixed_dense"
just_fixed_dense_times[!, :matrix] .= "fixed_dense"

combined_just_vals = vcat(
    just_sparse_vals,
    just_dense_vals,
    just_fixed_dense_vals;
    cols=:union,
)
combined_just_times = vcat(
    just_sparse_times,
    just_dense_times,
    just_fixed_dense_times;
    cols=:union,
)

CSV.write(sim_resultpath("sparse_dense_vals_just.csv"), combined_just_vals)
CSV.write(sim_resultpath("sparse_dense_times_just.csv"), combined_just_times)
